...
async def on_ready():
    ...
    if con.is_connected():
        contador = 0
        sql = ("SELECT csla.id AS COUNT, cliente.cliente AS cliente, psla.nome AS prioridade, tsla.nome AS tela, csla.prazo, csla.descricao, modu.modulo AS modulo, IF(csla.`direcionamento` = 'S', 'SUPORTE', 'DESENVOLVIMENTO') AS direcionamento FROM tb_chamados_sla csla JOIN tb_prioridade_sla psla ON csla.prioridade = psla.id JOIN tb_tela_sla tsla ON tsla.id = csla.tela JOIN tb_cliente cliente ON cliente.id = csla.cliente JOIN tb_modulo modu ON tsla.modulo = modu.id WHERE direcionamento = 'S' AND csla.status = '1' ORDER BY csla.id DESC LIMIT 1;")
        cursor.execute(sql)
        for (count) in cursor:
            contador = count[0]
        global countGlobal
    ...
    while True:
        ...
        if con.is_connected():
            ...
            if(int(contador) > int(countGlobal)):
                ...
                channel = discord.utils.get(client.get_all_channels(), id=921394759426252810)
                countGlobal = contador
